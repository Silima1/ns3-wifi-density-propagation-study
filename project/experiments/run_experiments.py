...
MATRIX_FILE = PROJECT_DIR / "config" / "experimental_matrix.csv"
...
def load_matrix(matrix_file: Path):
    if not matrix_file.exists():
        raise FileNotFoundError(f"Experimental matrix not found: {matrix_file}")

    with open(matrix_file, newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))

    if not rows:
        raise ValueError(f"Experimental matrix is empty: {matrix_file}")
    ...
    if missing:
        raise ValueError(
            f"Experimental matrix is missing required columns: {sorted(missing)}"
        )
...
def main() -> None:
    ...
    print(f"[INFO] Experimental matrix : {MATRIX_FILE}")
    print(f"[INFO] Simulation script   : {SIM_SCRIPT}")
    print(f"[INFO] Total runs to execute: {total_runs}")

    failed_runs = []

    with tqdm(total=total_runs, desc="Running experiments", unit="run") as pbar:
